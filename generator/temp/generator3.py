...
# Convert sets to lists and mark registered keys (manually updated later)
mappings_to_save = {
    "registered": {
        "add_to_cart": ["product_id", "product_name"],  # example; you update these manually
        "purchase": [],
    },
    "to_register": {}
}

for event_name, keys in event_mappings.items():
    already = set(mappings_to_save["registered"].get(event_name, []))
    to_register = sorted(list(keys - already))
    mappings_to_save["to_register"][event_name] = to_register
